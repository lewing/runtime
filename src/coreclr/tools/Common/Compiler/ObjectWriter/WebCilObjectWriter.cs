    ...
    internal sealed class WebCilObjectWriter : WasmObjectWriter
    {
        ...
        private protected override void EmitObjectFile(Stream outputFileStream)
        {
           ...
           *********************************************************************/
            ...
            // Data segment for the webcil payload contents. A self-installing image emits it as active at the
            // host-supplied image base so the engine installs it at instantiation; a component stub
            // keeps it passive because it is extracted from the file rather than instantiated.
            WasmDataSegment webcilContentsSegment = IsSelfInstallingImage
                ? new WasmDataSegment(webcilStream, new Utf8String("webcilPayload"),
                    WasmDataSegmentType.Active, ImageBaseOffsetExpr)
                : new WasmDataSegment(webcilStream, new Utf8String("webcilPayload"),
                    WasmDataSegmentType.Passive, null);
